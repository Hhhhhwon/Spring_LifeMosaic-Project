		...
		<div id="content">
			...
			<!-- 회원의 정보들 설정 -->
				<div class="card" style="margin-left:30px; margin-right:30px;">
					...
					<div style="margin-left:30px; margin:30px;font-weight:bold; font-size:20px;">
					 <img style="margin-right:10px;" class="profileImg " src="${settingImgPage}?fileName=${profile_url}" name="profileImg" id="profileImg" alt="프로필 사진">
					 	${nickname}
					</div>
					<hr>
					<div style=" width:50px; margin-left:20px; font-weight:bold; color:#711DB0;">
					 게시물
					<hr style="margin:0; margin-top:5px; width:47px; border:2px solid #711DB0;">
					</div>
				</div>
				<!-- 회원의 게시물들을 보여주기 -->
				<c:choose>
					<c:when test="${not empty postCount and postCount != 0 }">
						<c:forEach var="post" items="${post}">
							
							<div class="card" style="border:none; margin:30px;">
								<div style=" text-align: right; " >
									<span class="mb-2 text-secondary" style=" font-size: small; margin-right:10px;">작성일자: ${post.post_created_time}</span>
								</div>
								<div>
								...
								<nav>
									<ul class="pagination pagination-sm" style="background-color: #F3F1F5;">
										<c:forEach var="pageNumber" begin="1" end="${pagesCount }">
											<c:url var="settingMemberPostListPage" value="/member/memberPostList">
												<c:param name='nickname' value='${nickname}'/>
												<c:param name="currentPage" value="${pageNumber }"></c:param>
											</c:url>
											<li class="page-item ${pageNumber == currentPage ? 'active' :'' }" style="display: inline;" >
												<a class="page-link" href="${settingMemberPostListPage}"  
